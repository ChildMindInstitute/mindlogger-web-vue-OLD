...
export default {
  ...
  methods: {
    ...
    evaluateScoringLogic() {
      const scoringLogic = (this.activity['https://raw.githubusercontent.com/ReproNim/schema-standardization/master/terms/scoringLogic'][0]['@value']).split('= ')[1];
      if (this.responses) {
        let str = '';
    ...
    nextQuestion(idx, skip, dontKnow) {
      if (skip) {
        ...
        // if (!_.isEmpty(this.activity['https://raw.githubusercontent.com/ReproNim/schema-standardization/master/terms/scoringLogic'])) {
    ...
    setResponse(value, index) {
      ...
      // if (this.activity['https://raw.githubusercontent.com/ReproNim/schema-standardization/master/terms/branchLogic']) {
    ...
    responseMapper(responses) {
      ...
      if (this.activity['https://raw.githubusercontent.com/ReproNim/schema-standardization/master/terms/variableMap']) {
        const vmap = this.activity['https://raw.githubusercontent.com/ReproNim/schema-standardization/master/terms/variableMap'][0]['@list'];
        const keyArr = _.map(vmap, (v) => {
          const key = v['https://raw.githubusercontent.com/ReproNim/schema-standardization/master/terms/isAbout'][0]['@id'];
          const qId = v['https://raw.githubusercontent.com/ReproNim/schema-standardization/master/terms/variableName'][0]['@value'];
    ...
    getVisibility(responses) {
      ...
      if (!_.isEmpty(this.activity['https://raw.githubusercontent.com/ReproNim/schema-standardization/master/terms/visibility'])) {
        const visibilityMapper = {};
        _.map(this.activity['https://raw.githubusercontent.com/ReproNim/schema-standardization/master/terms/visibility'], (a) => {
          let val = a['@value'];
    ...
    order() {
      if (this.activity['https://raw.githubusercontent.com/ReproNim/schema-standardization/master/terms/shuffle'][0]['@value']) { // when shuffle is true
        const orderList = this.activity['https://raw.githubusercontent.com/ReproNim/schema-standardization/master/terms/order'][0]['@list'];
        ...
        return newList;
      } return this.activity['https://raw.githubusercontent.com/ReproNim/schema-standardization/master/terms/order'][0]['@list'];
    },
  },
  watch: {
    $route() {
  ...
  computed: {
      ...
      if (this.$store) {
        ...
        if (state.activities.length && state.activityIndex != null) {
          if (state.activities[state.activityIndex].activity) {
            ...
            const actList = currentActivity['https://raw.githubusercontent.com/ReproNim/schema-standardization/master/terms/order'][0]['@list'];
            return actList;
          }
        }
      }
    ...
    context() {
      /* eslint-disable */
      if (this.activity['https://raw.githubusercontent.com/ReproNim/schema-standardization/master/terms/order']) {
    ...
    findPassOptions() {
      if (this.activity['https://raw.githubusercontent.com/ReproNim/schema-standardization/master/terms/allow']) {
        let isSkip = false;
        let isDontKnow = false;
        _.map(this.activity['https://raw.githubusercontent.com/ReproNim/schema-standardization/master/terms/allow'][0]['@list'], s => {
          if (s['@id'] === "https://raw.githubusercontent.com/ReproNim/schema-standardization/master/terms/refused_to_answer") {
            isSkip = true;
          } else if (s['@id'] === "https://raw.githubusercontent.com/ReproNim/schema-standardization/master/terms/dont_know_answer") {
            isDontKnow = true;
          }
        });
        return {
