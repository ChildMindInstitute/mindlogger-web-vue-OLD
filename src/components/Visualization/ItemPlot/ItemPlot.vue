  ...
  computed: {
    inputType() {
      if (!_.isEmpty(this.metadata)) {
        return this.metadata['https://raw.githubusercontent.com/ReproNim/schema-standardization/master/terms/inputType'][0]['@value'];
      }
      return null;
    },
    description() {
  ...
  methods: {
    getItemMetadata() {
      jsonld.expand(this.itemUrl).then((resp) => {
        ...
        if (this.metadata['https://raw.githubusercontent.com/ReproNim/schema-standardization/master/terms/valueconstraints'][0]['@id']) {
          jsonld.expand(this.metadata['https://raw.githubusercontent.com/ReproNim/schema-standardization/master/terms/valueconstraints'][0]['@id']).then((vc) => {
            this.metadata['https://raw.githubusercontent.com/ReproNim/schema-standardization/master/terms/valueconstraints'] = vc;
    ...
    getMeaningfulResponses() {
      const output = {};
      const vc = this.metadata['https://raw.githubusercontent.com/ReproNim/schema-standardization/master/terms/valueconstraints'];
